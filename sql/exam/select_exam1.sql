...
select ename, sal from emp where sal between 2000 and 3000 order by sal;
...
-- like는 괄호를 먼저 치고 안에서 관계연산자를 사용
select ename from emp where (ename like '%A%' and ename like '%E%');

-- 14. 담당업무가 CLERK, 또는 SALESMAN이면서 급여가 $1600, $950 또는 $1300이 아닌 사원의 이름, 담당업무, 급여를 출력하시오.
select ename, job, sal from emp where job in('CLERK','SALESMAN') and (sal in(1600,950) or sal<>1300) order by job;
